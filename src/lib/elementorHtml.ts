...
.elementor-nav-menu a{display:flex;align-items:center;gap:5px;line-height:1.4;padding:12px 16px}
...
.elx-carousel-track{display:flex;overflow:hidden;width:100%;position:relative}
.elx-carousel-track figure{margin:0;flex:1 1 0;min-width:0;position:relative}
.elx-carousel-track img{width:100%;height:100%;display:block;object-fit:cover}
.elx-carousel-track figcaption{position:absolute;left:0;right:0;bottom:0;padding:26px 18px 12px;background:linear-gradient(transparent,rgba(0,0,0,.72));color:#fff;font-size:15px;font-weight:600}
.elx-figcap{position:relative;margin:0}
.elx-figcap figcaption{position:absolute;left:0;right:0;bottom:0;padding:14px 14px 10px;background:linear-gradient(transparent,rgba(0,0,0,.78));color:#fff;font-weight:600;font-size:15px}
.elx-gallery{display:grid;gap:14px}
.elx-gallery figure{margin:0;position:relative}
.elx-gallery img{width:100%;aspect-ratio:1/1;object-fit:cover;border-radius:6px;display:block}
.elx-gallery figcaption{position:absolute;left:0;right:0;bottom:0;padding:18px 10px 8px;background:linear-gradient(transparent,rgba(0,0,0,.65));color:#fff;font-size:12px;border-radius:0 0 6px 6px}
...
function renderWidget(w: ElxElement, ctx: { css: string[]; design: RenderDesign }): string {
  ...
  switch (type) {
    ...
    case 'image': {
      ...
      const cropH = num(s._elx_img_height);
      if (cropH) ctx.css.push(`${sel} img{height:${cropH}px;width:100%;object-fit:cover;display:block}`);
      const rad = num(s._elx_img_radius);
      if (rad) ctx.css.push(`${sel} img{border-radius:${rad}px}`);
      const opacity = num(s._elx_opacity);
      if (opacity !== null && opacity < 0.99) ctx.css.push(`${sel} img{opacity:${opacity}}`);
      ...
      const captioned = s._elx_caption
        ...
        : inner;
      const linked =
        s.link_to === 'custom' && link?.url ? `<a href="${esc(link.url)}" target="_blank" rel="noreferrer">${captioned}</a>` : captioned;
    ...
    case 'image-carousel': {
      ...
      const captions = (s._elx_captions as string[] | undefined) || [];
      const slideH = num(s._elx_slide_height);
      const widthPct = Math.round(10000 / show) / 100;
      const heightCss = slideH ? `height:${slideH}px;object-fit:cover` : show >= 3 ? 'height:200px;object-fit:cover' : 'height:auto';
      ctx.css.push(
        `${sel} .elx-carousel-track figure{flex:0 0 ${widthPct}%}${sel} .elx-carousel-track img{${heightCss}}${show > 1 ? `${sel} .elx-carousel-track{gap:${show >= 3 ? 10 : 4}px}` : ''}`
      );
      const slides = imgs
        .slice(0, Math.max(show + 1, 3))
        .map((u, i) => {
          const cap = captions[i] ? `<figcaption>${esc(captions[i])}</figcaption>` : '';
          return `<figure><img src="${esc(u)}" alt="${esc(captions[i] || '')}" loading="${i === 0 ? 'eager' : 'lazy'}">${cap}</figure>`;
        })
        .join('');
      const arrows =
    ...
    case 'image-gallery': {
      const items = (s._elx_gallery || []) as Array<{ url?: string; caption?: string; alt?: string }>;
      const cols = Math.max(1, Math.min(6, parseInt(String(s.gallery_columns || '3'), 10) || 3));
      ctx.css.push(`${sel} .elx-gallery{grid-template-columns:repeat(${cols},1fr)}`);
      const cells = items
        .slice(0, 18)
        .map(
          (it) =>
            `<figure><img src="${esc(it.url)}" alt="${esc(it.caption || it.alt || '')}" loading="lazy">${it.caption ? `<figcaption>${esc(it.caption)}</figcaption>` : ''}</figure>`
        )
        .join('');
      return `<div class="${cls}"><div class="elementor-widget-container"><div class="elx-gallery">${cells}</div></div></div>`;
    }
...
// Card chrome recovered by the engine (radius / shadow / border) — painted as
// custom properties on the column or container element it belongs to.
function cardChromeCss(sel: string, s: ElxSettings): string[] {
  const rules: string[] = [];
  const radius = num(s._elx_radius);
  if (radius) rules.push(`${sel}{border-radius:${radius}px;overflow:hidden}`);
  const shadow = s._elx_shadow;
  if (shadow) rules.push(`${sel}{box-shadow:${shadow}}`);
  const bw = s._elx_border as { width?: number | string; color?: string } | undefined;
  if (bw && bw.width) rules.push(`${sel}{border:${bw.width}px solid ${bw.color || '#E4E4E4'}}`);
  return rules;
}
...
function columnCss(col: ElxElement, colsTotal: number): string {
  ...
  rules.push(...cardChromeCss(sel, s));
  const bg = bgCss(s);
